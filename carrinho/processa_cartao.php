...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: cadastrar_cartao.php');
    exit;
}
...
if (empty($numero_cartao) || !preg_match('/^\d{16}$/', $numero_cartao) || empty($nome_titular) || !preg_match('/^(0[1-9]|1[0-2])\/\d{2}$/', $data_validade) || empty($cvv) || !preg_match('/^\d{3,4}$/', $cvv)) {
    header('Location: cadastrar_cartao.php?error=invalid_data');
    exit;
}
...
try {
    ...
    header('Location: cadastrar_cartao.php?error=encryption_failed');
    exit;
}
...
try {
    ...
    if ($updateResult->getModifiedCount() > 0 || $updateResult->getMatchedCount() > 0) {
        ...
        header('Location: cadastrar_cartao.php?error=user_not_found');
        exit;
    }
    ...
    header('Location: cadastrar_cartao.php?error=db_error');
    exit;
}
